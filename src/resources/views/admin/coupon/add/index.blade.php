  ...
  <section class="body_main">
    <div class="container">
      <div class="box box_600">
        <div class="head_box">
          <div class="text">
            <p class="h1">クーポンを追加する</p>
            <p class="description">クーポン情報を入力してください</p>
            <p class="description">「<span style="color: #FF0000;">*</span>」は必須項目ですので必ず入力してください。</p>
          </div>
        </div>
        <div class="body_box">
          {{ Form::open(['route'=>'admin.coupon.add', 'method'=>'post','enctype'=>'multipart/form-data','files'=> true]) }}
              ...
              <div class="form_data">
                <div class="wrap_input wrap_radio">
                  @foreach(CustomConst::COUPON_TYPE as $key => $val)
                    ...
                    @else
                      {{ Form::radio('type', $key, false,['id' => 'radio'.$key]) }}
                      {{ Form::label('radio'.$key, $val) }}
                    @endif
                  @endforeach
                </div>
          ...
          <div class="btnarea_form">
            <a href="{{ route('admin.coupon') }}" class="btn_gray">キャンセル</a>
